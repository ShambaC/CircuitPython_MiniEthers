...
def deepcopy(obj, _memo=None):
    """A minimal deepcopy replacement that handles nested lists, tuples, dicts, and primitive types.
    It's intentionally small to work on constrained devices. It does not handle custom objects or
    objects with __getstate__/__setstate__.
    """
    if _memo is None:
        _memo = {}
    ...
    if isinstance(obj, list):
        ...
        _memo[obj_id] = res
        for item in obj:
            res.append(deepcopy(item, _memo))
        return res

    # Tuples
    if isinstance(obj, tuple):
        res = tuple(deepcopy(x, _memo) for x in obj)
        _memo[obj_id] = res
        return res
