...
class Wallet:
    ...
    def _calculate_recovery_id(self, message_hash, r, s):
        ...
        for recovery_id in range(4):
            recovered_pub_x, recovered_pub_y = self._recover_public_key(message_hash, r, s, recovery_id)
    ...
    def sign_message(self, message):
        ...
        return {
            'r': '0x' + self._int_to_hex(r),
            's': '0x' + self._int_to_hex(s),
            'v': v
        }
        ...
        if type_name == 'string':
            ...
            for item in value:
                if element_type in types:
                    encoded_elements.append(Wallet._hash_struct(element_type, item, types).to_bytes(32, 'big'))
                else:
